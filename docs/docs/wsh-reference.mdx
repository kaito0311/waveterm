...
- `--notify string` - also show a native OS notification (Windows, macOS, Linux) with this message
- `--notify-title string` - title for the `--notify` notification (default `"Wave Terminal"`)
...
# Set a badge and also show a native OS notification
wsh badge triangle-exclamation --color red --notify "Build failed" --notify-title "CI"
