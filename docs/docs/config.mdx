...
| preview:defaultdir <VersionBadge version="v0.15" />                  | string   | sets the default directory that new file explorer (preview) blocks open to when no directory is otherwise specified (defaults to `"~"`)                                                                                                                       |
...
## File Bookmarks Configuration <VersionBadge version="v0.15" />

File Bookmarks let you define quick-navigation shortcuts for the file explorer (preview) view. They are stored in a JSON file (`filebookmarks.json`) as a key-value map where the key (`id`) is an arbitrary identifier for the bookmark. In the file explorer, long-click (or right-click) the folder icon in the block header to see the "Go to" menu built from these bookmarks, along with an "Edit Bookmarks..." item that opens `filebookmarks.json` directly.

### File Bookmark Structure

Each bookmark follows this structure (only `path` is required):
...
| Field         | Type    | Description                                                                                                     |
| ------------- | ------- | ----------------------------------------------------------------------------------------------------------------- |
| path          | string  | **Required.** The directory path for the bookmark (supports `~`).                                               |
| connection    | string  | **Optional.** The connection this path belongs to (e.g. `wsl://Ubuntu`, or an SSH connection name). Omit for a local path. |
| label         | string  | **Optional.** A display label for the bookmark (falls back to `path`).                                          |
| display:order | float64 | **Optional.** Defines the order in which bookmarks appear.                                                      |

### Example `filebookmarks.json`
...
- Bookmarks are sorted based on `display:order` (if provided), otherwise by label/path.
- Wave Terminal ships with default bookmarks for Home, Desktop, Downloads, Documents, and Root; add a `filebookmarks.json` in your config directory to add, remove, or override them.
- Selecting a bookmark switches the block to that bookmark's `connection` (or back to local if none is set), then navigates to `path` on that connection.
- Clicking the star icon in the file explorer header bookmarks (or un-bookmarks) the directory currently being viewed, on whatever connection it's currently viewed from.
